...
def index(request):
    ...
    if len(c01) == 0:
        ...
        c03.save()
    ### Get User Types
    c04=userType.objects.all()
    ### Get College Email
    c05=Person.objects.all()
    ### Context Info
    context = {
        "allUserType" : c04,
        "allEmail" : c05,
...
def admin(request):
    ...
    if 'adminRight' not in request.session:
        return redirect('/campus3')
    ### Get All User Info
    c41=Person.objects.all()
    ### Get the User Info
    c42=Person.objects.get(id=request.session['id'])
    ### Context Info
    context = {
        "allPersons" : c41,
        "personInfo" : c42,
...
def dashboard(request,userID):
    ...
    if str(request.session['id']) != str(userID):
        return redirect('/campus3')
    ### Get the User Info
    c51=Person.objects.filter(id=userID)
    ### Get User Types
    c52=userType.objects.all()
    ### Get Group Types
    c53=userGroup.objects.filter(person_ID=userID)
    ## Get Building Info
    c55=Building.objects.filter(ID_building=userID)
    ## Get Parking Info
    c56=Parking.objects.filter(ID_parking=userID)
    ## Get Meal Cards
    c57=MealCardType.objects.all()
    ## Get Meal Plans
    c58=MealGroup.objects.filter(ID_meal=userID)
    ### Total Meal Amout
    totalAmt = 0
    for zz in c58:
        c58a=MealCardType.objects.get(id=zz.ID_mealType.id)
        totalAmt = totalAmt + c58a.CardAmt
    ### Get Person Info
    c59=Person.objects.get(id=userID)
    ## Get date
...
def user(request,userID):
    ...
    ### Calculate Age
    c62=Person.objects.get(id=userID)
    temp_today = datetime.datetime.now().date()
    ...
    ## Get Building Info
    c64=Building.objects.filter(ID_building=userID)
    ## Get Parking Info
    c65=Parking.objects.filter(ID_parking=userID)
    ## Get Meal Cards
    c66=MealCardType.objects.all()
    ## Get Meal Plans
    c67=MealGroup.objects.filter(ID_meal=userID)
    ### Total Meal Amout
    totalAmt = 0
    for zz in c67:
        c67a=MealCardType.objects.get(id=zz.ID_mealType.id)
...
def addUserGroup(request,userID):
    ...
    if request.method == "POST":
        ...
        if str(temp_group) == '0':
            ...
            c72=userType.objects.get(id=request.POST.get('inputUserType'))
            ### Insert UserType Into DB
            userGroup.objects.create(person_ID=c71,userType_ID=c72)
    ### Check AdminRight
    if str(request.session['adminRight']) == 'False':
        return redirect('/campus3/dashboard/'+userID)
    else:
...
def services(request,userID):
    ...
    temp_dayFormat= temp_extraDay.strftime("%Y-%m-%d")
    ### Get the User Info
    c81=Person.objects.filter(id=userID)
    ### Get User Types
    c82=userType.objects.all()
    ### Get Group Types
    c83=userGroup.objects.filter(person_ID=userID)
    ### Filter Out UserType From GroupType
    c82a=set(c82)
    c83a=set(c83)
    for xx in c83a:
        # print xx.userType_ID.id
        c84=userType.objects.filter(id=xx.id)
        c82a=c82a.difference(c84)
    ## Get Building Info
    c85=Building.objects.filter(ID_building=userID)
    ## Get Parking Info
    c86=Parking.objects.filter(ID_parking=userID)
    ## Get Meal Cards
    c87=MealCardType.objects.all()
    ## Get Meal Plans
    c88=MealGroup.objects.filter(ID_meal=userID)
    ### Total Meal Amout
    totalAmt = 0
    for zz in c88:
        c88a=MealCardType.objects.get(id=zz.ID_mealType.id)
        totalAmt = totalAmt + c88a.CardAmt
    ### Calculate Age
    c89=Person.objects.get(id=userID)
    temp_today = datetime.datetime.now().date()
...
def addMeal(request,userID):
    ...
    if request.method == "POST":
        ...
        if str(temp_group) == '0':
            ...
            ### Get the Record
            c91=Person.objects.get(id=userID)
            c92=MealCardType.objects.get(id=request.POST.get('inputMealCard'))
            ### Insert MealGroup Into DB
            MealGroup.objects.create(ID_meal=c91,ID_mealType=c92)
    ### Check AdminRight
    if str(request.session['adminRight']) == 'False':
        return redirect('/campus3/dashboard/'+userID)
    else:
...
def addBuilding(request,userID):
    ...
    if request.method == "POST":
        ...
        if len(errors):
            ...
            ### Get userID
            c101=Person.objects.get(id=userID)
            ### Insert Into DB
            Building.objects.create(
                ID_building=c101,
                buildingName=request.POST.get('inputBuilding'), 
...
def addParking(request,userID):
    ...
    if request.method == "POST":
        ...
        if len(errors):
            ...
            ### Get userID
            c111=Person.objects.get(id=userID)
...
def deleteGroup(request,userID,noID):
    ...
    c121=userGroup.objects.get(id=noID)
    c121.delete()
    ### Check AdminRight
    if str(request.session['adminRight']) == 'False':
        return redirect('/campus3/dashboard/'+userID)
...
def deleteMeal(request,userID,noID):
    ...
    c131=MealGroup.objects.get(id=noID)
    c131.delete()
    ### Check AdminRight
    if str(request.session['adminRight']) == 'False':
        return redirect('/campus3/dashboard/'+userID)
...
def deleteBuilding(request,userID,noID):
    ...
    c141=Building.objects.get(id=noID)
    c141.delete()
    ### Check AdminRight
    if str(request.session['adminRight']) == 'False':
        return redirect('/campus3/dashboard/'+userID)
...
def deleteParking(request,userID,noID):
    ...
    if 'adminRight' not in request.session:
        return redirect('/campus3')
    ### Get Parking 
    c151=Parking.objects.get(id=noID)
    c151.delete()
    ### Check AdminRight
    if str(request.session['adminRight']) == 'False':
        return redirect('/campus3/dashboard/'+userID)
...
def updateExtNum(request,userID):
    ...
    if request.method == "POST":
        ...
        if len(errors):
            ...
            c171=Person.objects.get(id=userID)
...
def updatePersonEmail(request,userID):
    ...
    if request.method == "POST":
        ...
        if len(errors):
            ...
            c181=Person.objects.get(id=userID)
...
def hack(request,userID):
    ...
    if str(request.session['id']) != str(userID):
        ...
        if str(request.session['adminRight']) == 'False':
            ...
            return redirect('/campus3/admin')
    ### Get the User Info
    c191=Person.objects.get(id=userID)
    ### Context Info
    context = {
        "personInfo" : c191,
...
def hacking(request,userID):
    ...
    if str(request.session['id']) != str(userID):
        ...
        if str(request.session['adminRight']) == 'False':
            ...
            return redirect('/campus3/admin')
    errors = {}
    c201=Person.objects.get(id=userID)
    if str(request.session['adminRight']) == 'False':
...
def destory(request,userID):
    ...
    summary = " "
    ### Delete User group
    c211=userGroup.objects.filter(person_ID=userID)
    if len(c211) > 0:
        summary = summary + str(len(c211)) +" record(s) of Group. "
    ### Delete Meal group
    c212=MealGroup.objects.filter(ID_meal=userID)
    if len(c212) > 0:
        summary = summary + str(len(c212)) +" record(s) of Meal. "
    ### Delete Building group
    c213=Building.objects.filter(ID_building=userID)
    if len(c213) > 0:
        summary = summary + str(len(c213)) +" record(s) of Building. "
    ### Delete Parking group
    c214=Parking.objects.filter(ID_parking=userID)
    if len(c214) > 0:
        summary = summary + str(len(c214)) +" record(s) of Parking Permit. "
    ### Delete User
    c215=Person.objects.filter(id=userID)
    if len(c215) > 0:
        c215a=Person.objects.get(id=userID)
...
def deleteGroupType(request,noID):
    ...
    errors = {}
    c221=userGroup.objects.filter(userType_ID=noID)
    c222=userType.objects.get(id=noID)
    if len(c221) > 0:
...
def deleteMealType(request,noID):
    ...
    errors = {}
    c231=MealGroup.objects.filter(ID_mealType=noID)
    c232=MealCardType.objects.get(id=noID)
    if len(c231) > 0:
